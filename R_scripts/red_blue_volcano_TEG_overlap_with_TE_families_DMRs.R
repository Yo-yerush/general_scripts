...
grouped_families <- function(superFamily=NULL, Family=NULL, SP_df=F) { # 'SP_df' for results data frame of super-family
  ...
  if (SP_df) {
    x = TEG_df %>% filter(Transposon_Super_Family == superFamily) %>% arrange(pValue)
  }
  
  # for family results
  if (!is.null(Family)) {
    x = TEG_df %>% filter(Transposon_Family == Family) %>% arrange(pValue)
  }
  ###
...
mydf$geneCat <- with(mydf, ifelse(pValue < 0.05 & log2FoldChange > 0, "Upregulated",
                                  ifelse(pValue < 0.05 & log2FoldChange < 0, "Downregulated", "nonDE")))
mydf$geneCat <- factor(mydf$geneCat, levels = c("Upregulated", "Downregulated", "nonDE"))

x_pos = 0.4

svg("C:/Users/yonatany/Migal/Rachel Amir Team - General/yonatan/methionine/mto1_paper/red_blue_DMRs_all_cntx_on_retro-TEGs_volcano.svg",
    width = 3.8, height = 2, family = "serif")

ggplot(mydf, aes_string(x = "log2FoldChange", y = "-log10(pValue)")) +
