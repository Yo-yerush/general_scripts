...
main_output_directory <- "C:/Users/yonatany/Migal/Rachel Amir Team - General/yonatan/methionine/mto1_paper/linear_plot_TEGs_width/"
        ...
        tryCatch(
            {
                ...
                if (context == "CG") {
                    ...
                    yaxis_txt_var <- element_text(size = 10.5)
                } else {
                ...
                # if (context == "CHG") {
                #     xaxis_text <- "Methylation level"
                # } else {
                #     xaxis_text <- " "
                # }

                xaxis_txt_var <- element_text(size = 10.5)
                xaxis_var <- element_text(size = 18)
                # xaxis_var <- element_text(size = 16)

                title_var <- element_text(size = 11)
                context_label <- context

                #### plot ####
                p <- ggplot(plot_df, aes(x = Meth, y = normCounts, color = genotype))

                p_points <- p + geom_point(aes(alpha = 0.85), size = 0.005, show.legend = FALSE) +
                    stat_smooth(method = "glm.nb", formula = y ~ x, linewidth = 0.7) +
                    scale_y_continuous(
                    ...
                    scale_x_continuous(
                        breaks = scales::rescale(c(0, 1), to = range(plot_df$Meth)),
                        labels = c(0, round(max(plot_df$Meth), 1))
                    ) +
                    coord_cartesian(ylim = c(0, max(plot_df$normCounts))) +
...
tiff(paste0(path_2_save.2, "lm.stats.plot.", var2_name, ".tif"), width = 4, height = 1.8, units = "in", res = 600, family = "serif") # width = 4
...
do.call(multiplot, c(
    ...
    list(
        ...
        heights = c(1, 1, 1),
        widths = c(1, 0.67, 0.65)
    )
))

grid::grid.text("Methylation level", x = 0.58, y = 0.075, gp = grid::gpar(fontsize = 16, fontfamily = "serif"))
