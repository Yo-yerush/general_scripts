...
df_x <- as.data.frame(readxl::read_xlsx("C:/Users/yonatany/Migal/Rachel Amir Team - General/yonatan/methionine/NGS_merged_results/mto1_groups.xlsx",
    ...
    filter(grepl("DMT2|MET1|CMT", Symbol)) # keep only thise mentioned in the paper

y_max_plot <- 0.1 #max(df_x$RNA_log2FC)
...
g1 <- ggplot(data = df_x, aes(x = reorder(Symbol, RNA_log2FC), y = RNA_log2FC, fill = color)) +
    ...
    scale_y_continuous(expand = c(0, 0), limits = c(y_min_plot - adding_min, 0.5)) #y_max_plot + adding_max))
