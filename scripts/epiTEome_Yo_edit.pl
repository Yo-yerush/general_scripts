...
###################################################################
# edit this file couse of 'ngsutils' package configuration issues #
# using samtools 1.16 instead of 'bamutils' for filtering         #
# using 'seqtk subseq' to extract subsequences the fast1 files    #
#                                                                 #
# epiTEome tool:                                                  # 
...
sub step2{
	...
	if(-e $self->{option}->{unmappedFastqFile}->{prefix}.".step2.sort.bam"){ # Only for developpement
		...
		################################################## Yo edit
		my $whitelist = $self->{option}->{unmappedFastqFile}->{prefix} . '.step1.list';
		my $infq = $self->{option}->{unmappedFastqFile}->{file};
		my $outfq = $self->{option}->{unmappedFastqFile}->{prefix} . '.step1.fastq';

		# requires seqtk in PATH (conda: `conda install -c bioconda seqtk`)
		my @seqtk = ('seqtk', 'subseq', $infq, $whitelist, '>', $outfq);
		print STDERR join(" ", @seqtk), "\n";
		system(join " ", @seqtk) == 0 or die "DIE: seqtk subseq failed\n";

		if (-z $outfq) {
		    print STDERR "INFO $prog: $outfq is empty.\n";
		    return 0;
		}
		##################################################
...
sub pairedEndGrab {
	...
	######## Alternative approach for older samtools versions that don't support -N
	# my @bam_filter = (
	#     $self->{program}->{samtools}, 'view', '-h',
	#     $self->{fileName}->{mappingOutput}->{bam},
	#     '|', $self->{program}->{awk},
	#     "'NR==FNR{a[\$1]=1;next} /^@/ || \$1 in a'",
	#     $whitelist, '-',
	#     '|', $self->{program}->{samtools}, 'view', '-b', '-o', $fileName, '-'
	# );
	########
