...
for (treatment in c("mto1","mto3")) {
  ...
  if (F) {
      RNA_kegg = read.csv(paste0("C:/Users/yonye/Migal/Rachel Amir Team - General/yonatan/methionine/rnaseq_23/EC_kegg_maps/by_DEseq2/",treatment,"/",treatment,"_values_Cysteine and methionine metabolism_ath00270_by_DEseq2.csv"))
  RNA_kegg = RNA_kegg %>% filter(pValue < 0.05)
  RNA_kegg_transcriptsLoc = merge.data.frame(data.frame(locus_tag=RNA_kegg[,1]), tairs2merge, by = "locus_tag")
  RNA_kegg_transcriptsLoc = RNA_kegg_transcriptsLoc[,c("seqnames","start","end","locus_tag")] %>% filter(!duplicated(locus_tag))
  }
  
  # RNAseq methionin biosynthesis (KEGG) res file
  ...
  RNA_transcriptsLoc = merge.data.frame(data.frame(locus_tag=RNA_TEs[,1]), tairs2merge, by = "locus_tag")
  RNA_transcriptsLoc = RNA_transcriptsLoc[,c("seqnames","start","end","locus_tag")]
  ...
  for (context in c("CG","CHG","CHH")) {
      ...
      dir.create("C:/Users/yonye/Migal/Rachel Amir Team - General/yonatan/methionine/NGS_merged_results/circular_plot_res/DMRs_TEs_expression/", showWarnings = F)
      dir.create(paste0("C:/Users/yonye/Migal/Rachel Amir Team - General/yonatan/methionine/NGS_merged_results/circular_plot_res/DMRs_TEs_expression/",treatment), showWarnings = F)
      #####################################
      ...
      ############# the plot #############
      svg(paste0("C:/Users/yonye/Migal/Rachel Amir Team - General/yonatan/methionine/NGS_merged_results/circular_plot_res/DMRs_TEs_expression/",treatment,"/",treatment,"_DMRs_TEs_expression_",context,"_circular_plots.svg"), width = 3.25, height = 3.25, family = "serif")
      ...
      circos.genomicDensity(list(dmrs_file[dmrs_file$meth_log2FC > 0, 1:3],
                            ...
                            col = c("#FF000080","#304ed180"), border = T, track.height = 0.3)
      
      
      circos.genomicTrackPlotRegion(RNA_transcriptsLoc[,1:3], ylim = c(0,1), bg.col = "#fafcfc", bg.border = NA, panel.fun = function(region, value, ...) {
        circos.genomicLines(region, 1, type = "h", col = "black", lwd = 0.5)
      }, track.height = 0.05)
      
      #circos.genomicTrackPlotRegion(RNA_kegg_transcriptsLoc[,1:3], ylim = c(0,1), bg.col = "#fafcfc", bg.border = NA, panel.fun = function(region, value, ...) {
      #  circos.genomicLines(region, 1, type = "h", col = "green4", lwd = 1.5)
      #}, track.height = 0.05)
      
      circos.genomicDensity(TE_4_dens[1:3],
                            bg.col = "#fafcff", bg.border = NA, count_by = "number",
                            col = "#fcba0320", border = T, track.height = 0.2)
...
# legends
{
  ...
  svg(paste0("C:/Users/yonye/Migal/Rachel Amir Team - General/yonatan/methionine/NGS_merged_results/circular_plot_res/DMRs_TEs_expression/legends.svg"), width = 6, height = 5.75, family = "serif")
  #par(mfrow=c(3,1))
  ...
  #circos.lines(seq(0,1,1/(length(rndm_norm_hyper)-1)), rndm_norm_hyper, border = T)
  circos.text(text_tracks_pos, x = 0.1, y = 0.5, labels = "TEs", facing = "downward", adj = c(0, 0.5))
