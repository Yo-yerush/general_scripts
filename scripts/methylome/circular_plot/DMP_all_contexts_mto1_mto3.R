...
trimm_Chr <- function(gr_obj) {
  ...
  mto3_cx = paste0("/home/yoyerush/yo/methylome_pipeline/Bismark/res_310523/S",mto3_samples,"/methylation_extractor/S",mto3_samples,"_R1_bismark_bt2_pe.CX_report.txt")
  
  wt_methylationData <- mclapply(wt_cx, readBismark, mc.cores = 2)
  mto1_methylationData <- mclapply(mto1_cx, readBismark, mc.cores = 3)
  mto3_methylationData <- mclapply(mto3_cx, readBismark, mc.cores = 3)
  ...
  mto3_pool <- poolMethylationDatasets(GRangesList(mto3_methylationData))
  
  wt_pool = trimm_Chr(wt_pool)
  mto1_pool = trimm_Chr(mto1_pool)
  mto3_pool = trimm_Chr(mto3_pool)
  
  
  delta_list_mto1 = list(CG=NA, CHG=NA, CHH=NA)
  delta_list_mto3 = list(CG=NA, CHG=NA, CHH=NA)
  
  for (context in c("CG","CHG","CHH")) {
    
    profile_wt = GRanges()
    profile_mto1 = GRanges()
    profile_mto3 = GRanges()
    ...
    for (chr.n in as.character(wt_pool@seqnames@values)) {
      ...
      profile_mto3 <- c(profile_mto3, computeMethylationProfile(mto3_pool, regions, windowSize = 150000, context = context))
    }
    methylationProfiles_mto1 = profile_wt[,0]
    methylationProfiles_mto3 = profile_wt[,0]
    
    methylationProfiles_mto1$Proportion = profile_mto1$Proportion - profile_wt$Proportion
    methylationProfiles_mto3$Proportion = profile_mto3$Proportion - profile_wt$Proportion
    
    delta_df_mto1 = as.data.frame(methylationProfiles_mto1)[,c("seqnames","start","end","Proportion")]
    delta_df_mto3 = as.data.frame(methylationProfiles_mto3)[,c("seqnames","start","end","Proportion")]
    #delta_pos  = (delta_df$start + delta_df$end) / 2
    
    delta_list_mto1[[context]] = delta_df_mto1
    delta_list_mto3[[context]] = delta_df_mto3
  }
  ...
  #####################################
  #y_ranges = list(CG = range(c(delta_list_mto1[["CG"]]$Proportion, delta_list_mto3[["CG"]]$Proportion), na.rm = TRUE),
  #                CHG = range(c(delta_list_mto1[["CHG"]]$Proportion, delta_list_mto3[["CHG"]]$Proportion), na.rm = TRUE),
  #                CHH = range(c(delta_list_mto1[["CHH"]]$Proportion, delta_list_mto3[["CHH"]]$Proportion), na.rm = TRUE))
  ############# the plot #############
  svg(paste0("/home/yoyerush/yo/methylome_pipeline/circular_plots/DMP_all_contexts/mto1_mto3_all_contexts_circular_plots.svg"), width = 4.25, height = 4.25, family = "serif")
