...
# part1 = find interesting GOs
{
  ...
  all_interesting_tairs_0 = unique(yo[grep("^2\\.1\\.1\\.",yo$EC.number) ,"locus_tag"])
  all_interesting_tairs = data.frame(locus_tag = all_interesting_tairs_0,
                                     col = rep("red4", length(all_interesting_tairs_0)))
}
...
# part2 - circular corr plot
{
  ...
  dir.create("P:/yonatan/methionine/circular_plot_res/corr_TE_exp/EC_methyltranferase", showWarnings = F)
  for (treatment in c("mto1","mto3")) {
    for (context in c("CG","CHG","CHH")) {
      for (ann in c("genes","promoters")) {
        ...
        #####################################
        ############# the plot #############
        svg(paste0("P:/yonatan/methionine/circular_plot_res/corr_TE_exp/EC_methyltranferase/",treatment,"_",context,"_corr_",ann,"_circular_plots.svg"), width = 7.07, height = 7.40, family = "serif")
        ...
        circos.genomicTrackPlotRegion(Chr_name[,1:3], ylim = c(0,1), bg.col = "#fafcfc", bg.border = NA, panel.fun = function(region, value, ...) {
          ...
          circos.genomicLines(region, 1, type = "h", col = region_colors, lwd = 1.75)
        }, track.height = 0.12)
        
        circos.genomicDensity(RNA_transcriptsLoc[2:4], bg.col = "#fafcfc", bg.border = NA, count_by = "number", track.height = 0.175)
        circos.genomicDensity(TE_4_dens[1:3], bg.col = "#fafcfc", bg.border = NA, count_by = "number", track.height = 0.15)
        circos.clear()
...
# legends
{
  ...
  legend_titles_circular = c("Methylation","Expression","Overlapping TEs","EC 2.1.1.-","sig. Genes","TEs")
  ...
  svg(paste0("P:/yonatan/methionine/circular_plot_res/corr_TE_exp/EC_methyltranferase/EC_legends.svg"), width = 6, height = 5.75, family = "serif")
  #par(mfrow=c(3,1))
  ...
  circos.trackPlotRegion(factors = 1:tracks_total_size, ylim = c(0, 1), bg.border = NA, track.height = 0.12) 
  circos.updatePlotRegion(sector.index = legend_tracks_pos, bg.border = NA , bg.col = "#fafcfc", bg.lwd=0.2)
  circos.lines(rndm[3:7], rep(1,5), type = "h", col = all_interesting_tairs[1,2], lwd = 1.75)
