...
# part1 = find interesting GOs
{
  ...
  # save all_df data frame with description
  if (F) {
    for (i in 1:length(all_DFs)) {
      all_DFs_des = data.frame(locus_tag = unique(all_DFs[[i]][, "locus_tag"]))
      all_DFs_des = merge.data.frame(all_DFs_des, yo, by = "locus_tag")
    }
  }
...
# legends
{
  ...
  legend_titles_circular = c("Methylation","Expression","Overlapping TEs","Matching GO Terms","sig. Genes","TEs")
  ...
  circos.updatePlotRegion(sector.index = legend_tracks_pos, bg.border = NA , bg.col = "#fafcfc", bg.lwd=0.2)
  circos.lines(rndm[1:8], rep(1,8), type = "h", col = brewer.pal(n=9, "Set1")[-6], lwd = 1.75)
