...
uniP_0 = read.csv("P:/TAIR10.1/uniProt_description_files/uniprotkb_arabidopsis_AND_model_organis_2024_01_25.csv")[,-1]
...
# Araport11
ar11 = read.csv("P:/TAIR10.1/Araport11_description_files/Araport11_functional_descriptions_051023_without_C_M.txt", sep = "\t")
names(ar11)[1] = "locus_tag"
ar11$locus_tag = gsub("\\.[0-9]","",ar11$locus_tag)
ar11 = ar11[!duplicated(ar11$locus_tag),]


#tair2refseq = read.csv("P:/TAIR10/TAIR10 NCBI mapping files/TAIR10_NCBI_REFSEQ_mapping_RNA", sep = "\t", header = F)[,-1]
#names(tair2refseq) = c("transcript_id", "locus_tag")
#tair2refseq$locus_tag = gsub("\\.[0-9]","",tair2refseq$locus_tag)

gff3 = rtracklayer::import.gff3("P:/TAIR10.1/GCF_000001735.4_TAIR10.1_genomic_trimmed.gff3")
gff3_filtered = data.frame(gff3$type,
                           transcript_id = gsub("transcript:","",gff3$ID),
                           locus_tag = gff3$locus_tag,
                           gene = gff3$gene)
gff3_filtered = gff3_filtered[gff3_filtered$gff3.type == "transcript", -1]
...
write.csv(merge_descriptions,"P:/TAIR10.1/AT_description_file_070224.csv", row.names = F)
...
if (F) {
  uniP = read.csv("P:/TAIR10.1/uniProt_description_files/uniprotkb_arabidopsis_AND_model_organis_2024_01_25.csv")[,-1]
  names(uniP)[1] = "locus_tag"
