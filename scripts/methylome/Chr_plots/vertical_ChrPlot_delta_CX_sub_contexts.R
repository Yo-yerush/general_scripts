...
chromosome_plot <- function(delta_profile, # var1_profile, var2_profile,
  ...
  plot(pos, delta_profile[[1]]$Proportion,
    type = "o",
    ylim = c(ymin, ymax), xlab = "", ylab = paste0(cntx, " methylation (Δ)"),
  ...
  axis(2,
    at = c(ymin, 0, ymax), labels = c(round(ymin, 3), 0, round(ymax, 3)),
    cex.axis = 0.8, las = 1
  ) # ymax/2,
  ...
  text(
    x = par("usr")[2] - 0.125 * diff(par("usr")[1:2]),
    y = par("usr")[4],
...
for (treatment in c(
  # "wt", "mto1", "mto3", "EV_dCGS", "dCGS",
  "sseHigh", "sseLow", "EV_SSE"
)) {
  cat("\n*", treatment, "*\n\n")
  ...
  for (context_name in names(context_list)) {
    ...
    dir.create("/home/yoyerush/yo/methylome_pipeline/Chr_plots/delta_sub_contexts/vertical_plots", showWarnings = F)

    svg(paste0("/home/yoyerush/yo/methylome_pipeline/Chr_plots/delta_sub_contexts/vertical_plots/", treatment, "_sub.", context_name, "_delta_vertical_ChrPlot.svg"),
    ...
    for (chr_number in length(chr_names):1) {
      ...
      chromosome_plot(subContext_list_delta,
...
for (i_cntx in c("CHG", "CG", "CHH")) {
  svg(paste0("/home/yoyerush/yo/methylome_pipeline/Chr_plots/delta_sub_contexts/vertical_plots/", i_cntx, "_legend.svg"),
  ...
  par(lend = "square")
  legend("topleft",
    legend = context_list[[i_cntx]], title = i_cntx, title.adj = 0.25,
