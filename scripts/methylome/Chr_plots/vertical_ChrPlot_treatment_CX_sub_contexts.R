...
#source("C:/Users/yonye/Migal/Rachel Amir Team - General/yonatan/scripts/methylome/Chr_plots/ChrPlots_additional_scripts_180824.R")
...
chromosome_plot <- function(delta_profile, #var1_profile, var2_profile,
  ...
  col = paste0(brewer.pal(n = length(delta_profile), name = 'Set1'))#, "90") # 9 colors with 80% transparency
  ...
  plot(pos, delta_profile[[1]]$Proportion, type = "o", 
       ylim = c(ymin, ymax), xlab = "", ylab = paste0(cntx," methylation"), 
       col = col[1], pch = pch[1], lty = lty[1], lwd = lwd[1],
       yaxt = "n", 
       xaxt = "n", 
       main = ""
  )
  axis(2, at = c(ymin, ymax), labels = c(ymin, ymax), las = 1) # ymax/2,
  
  
  # lines for the rest of treatment vars
  for (i in 2:length(delta_profile)) {
    lines(pos, delta_profile[[i]]$Proportion, 
          type = "o", col = col[i], lty = lty[i], pch = pch[i], lwd = lwd[i])
  }
  ...
  # line at 0, mid and max (yaxis)
  lines(pos, rep(0,length(delta_profile[[1]])), 
        type = "o", col = "gray30", lty = 2, pch = 26)
  lines(pos, rep(ymax,length(delta_profile[[1]])), 
        type = "o", col = "gray40", lty = 2, pch = 26)
  #lines(pos, rep(ymax/2,length(delta_profile[[1]])), 
  #      type = "o", col = "gray30", lty = 2, pch = 26)
  
  # Adding text at the top left corner
  if (chr.n == 1) {
    text(x = par("usr")[1] + 0.125 * diff(par("usr")[1:2]), # Move a small fraction of the plot width to the right, 
         y = par("usr")[4], 
         labels = trnt, 
  ...
  # Adding text at the top right corner
  text(x = par("usr")[2] - 0.125 * diff(par("usr")[1:2]),
       y = par("usr")[4], 
       labels = paste0("Chr ",chr.n), 
       pos = 1,
       #offset = 1,
       cex = 0.9,
       adj = c(1, 1))
...
for (treatment in c("wt", "mto1", "mto3", "EV_dCGS", "dCGS", "sseHigh", "sseLow", "EV_SSE")) {
  cat("\n*", treatment, "*\n\n")

  ############# CX report #############

  if (treatment == "wt") {
    trnt_file_samples <- "mto1"
  } else if (treatment == "EV_dCGS") {
    trnt_file_samples <- "dCGS"
    treatment <- "EV"
  } else if (treatment == "sseHigh") {
    trnt_file_samples <- "sseHigh"
    treatment <- "SSE_high"
  } else if (treatment == "sseLow") {
    trnt_file_samples <- "sseLow"
    treatment <- "SSE_low"
  } else if (treatment == "EV_SSE") {
    trnt_file_samples <- "sseHigh"
    treatment <- "EV"
  }
  ...
  sample_df = sample_df[sample_df[,1] == treatment,]
  ...
  for (context_name in names(context_list)) {
    ...
    for (sub_context in context_list[[context_name]]) {
      ...
      for (chr.n in as.character(trnt_pool@seqnames@values)) {
        chr.vec = trnt_pool[trnt_pool@seqnames == chr.n]
        ...
        #profile_ctrl = c(profile_ctrl, MethylationProfile(ctrl_pool, regions, windowSize = 150000, context = sub_context))
    ...
    # if want y-axis will get at max/min values
    if (F) {
      max_proportions <- sapply(subContext_list_trnt, function(gr) {
        max(gr$Proportion)
      })
      
      min_proportions <- sapply(subContext_list_trnt, function(gr) {
        min(gr$Proportion)
      })
    ...
    dir.create("/home/yoyerush/yo/methylome_pipeline/Chr_plots/all_treatments_sub_contexts", showWarnings = F)
    dir.create("/home/yoyerush/yo/methylome_pipeline/Chr_plots/all_treatments_sub_contexts/vertical_plots", showWarnings = F)
    
    svg(paste0("/home/yoyerush/yo/methylome_pipeline/Chr_plots/all_treatments_sub_contexts/vertical_plots/",treatment,"_sub.",context_name,"_vertical_ChrPlot.svg"),
        #width = 8, height = 2, family = "serif")
        width = 2.5, height = 8, family = "serif")
    
    
    #par(mar = c(1,4,2,0))
    #par(fig=c(0,2,0,10)/10)
    #plot(runif(10),runif(10),xlim=c(0, 0.01), 
    #     ylim=c(y_cntx_min,y_cntx_max),axes=FALSE,type="n",ylab=paste(context_name," methylation"),xlab="")
    #axis(2, c(y_cntx_min ,y_cntx_max), lty = 1,
    #     labels = c(round(y_cntx_min,4), round(y_cntx_max,4)))
    #par(new=T)
    
    i=0
    u = 2#(10-i)/6
    for (chr_number in length(chr_names):1) {
      par(mar = c(0.25,4.25,0.25,0.25))
      par(fig=c(0,10,i,i+u)/10)
      #par(fig=c(i,i+u,0,10)/10)
      chromosome_plot(subContext_list_trnt, #subContext_list_ctrl, subContext_list_trnt,
    ...
    #legend("top", legend = context_list[[context_name]], 
...
################################################################################
### legend
h=0.5 #for height size
for (i_cntx in c("CHG","CG","CHH")) {
  
  svg(paste0("/home/yoyerush/yo/methylome_pipeline/Chr_plots/all_treatments_sub_contexts/vertical_plots/",i_cntx,"_legend.svg"),
      width = 0.6, height = h, family = "serif")
  par(mar = c(0,0,0,0)) #(bottom, left, top, right)
  plot.new()
  par(lend = "square")
  legend("topleft", legend = context_list[[i_cntx]], title = i_cntx, title.adj = 0.25,
         lty = 1, seg.len = 1, lwd = 2,
         col = paste0(brewer.pal(n = length(context_list[[i_cntx]]), name = 'Set1')),#, "90"), # 9 colors with 80% transparency
         #pch = 15, 
         bty = "n", cex = 0.5, title.cex = 0.75)
  dev.off()
