...
output_path <- "C:/Users/yonatany/Migal/Rachel Amir Team - General/yonatan/methionine/NGS_merged_results/TEGs_volcano_plots/DMRs/"
dir.create("C:/Users/yonatany/Migal/Rachel Amir Team - General/yonatan/methionine/NGS_merged_results/TEGs_volcano_plots/", showWarnings = FALSE)
dir.create(output_path, showWarnings = FALSE)

DMRs_volcano <- function(context, genotype) {
    ...
    if (context == "all") {
        DMR_file_0 <- rbind(
            read.csv(paste0("C:/Users/yonatany/Migal/Rachel Amir Team - General/yonatan/methionine/methylome_23/BSseq_results/", genotype, "/genome_annotation/CG/TEG_CG_genom_annotations.csv")),
            read.csv(paste0("C:/Users/yonatany/Migal/Rachel Amir Team - General/yonatan/methionine/methylome_23/BSseq_results/", genotype, "/genome_annotation/CHG/TEG_CHG_genom_annotations.csv")),
            read.csv(paste0("C:/Users/yonatany/Migal/Rachel Amir Team - General/yonatan/methionine/methylome_23/BSseq_results/", genotype, "/genome_annotation/CHH/TEG_CHH_genom_annotations.csv"))
        )
        ...
        DMR_file_0 <- read.csv(paste0("C:/Users/yonatany/Migal/Rachel Amir Team - General/yonatan/methionine/methylome_23/BSseq_results/", genotype, "/genome_annotation/", context, "/TEG_", context, "_genom_annotations.csv"))
    }
    ...
    vol_plot <- ggplot(mydf, aes_string(x = "log2FoldChange", y = "-log10(pValue)", color = "Transposon_Super_Family")) +
        geom_point(alpha = 0.4, size = 1) +
        # change axis titles
        xlab("log2(Fold-Change)") +
        ylab("-log10(p-Value)") +

        #ylim(c(0, 50)) + # mto3 - CHG
        #ylim(c(0, 50)) + # mto3 - CHH
        # theme_classic() + #
        theme_bw() +
        scale_colour_manual(
    ...
    svg(paste0(output_path, genotype, "_DMRs_on_retro-TEGs_volcano_", context, ".svg"),
...
####################################
for (genotype_loop in c("mto1_vs_wt", "mto3_vs_wt", "dCGS_vs_EV", "SSE_high_vs_EV", "SSE_low_vs_EV", "SSE_high_vs_SSE_low")) {
    for (context_loop in c("all", "nonCG", "CG", "CHG", "CHH")) {
        try(DMRs_volcano(context_loop, genotype_loop))

        cat(paste0(context_loop," contexts\n"))
    }
        cat(paste0("\n***\n",genotype_loop,"\n***\n\n"))
}
