ec_squere_plots <- function(treatment = NULL, annotation_type = "Genes", context = "all") { # "Promoters"
    library(scales)
    library(ggplot2)
    library(dplyr)
    res_dir <- "C:/Users/YonatanY/Migal/Rachel Amir Team - General/yonatan/methionine/methylome_23/BSseq_results/KEGG_pathway/with_EC"


    output_dir <- paste(res_dir, "square_plots", treatment, annotation_type, context, sep = "/")
    dir.create(paste0(res_dir, "/square_plots/"), showWarnings = F)
    dir.create(paste0(res_dir, "/square_plots/", treatment), showWarnings = F)
    dir.create(paste(res_dir, "square_plots", treatment, annotation_type, sep = "/"), showWarnings = F)
    ...
    for (n.map in 1:length(map_ids)) {
        ...
        file_full_name <- list.files(
            path = paste(res_dir, treatment, annotation_type, context, sep = "/"),
        ...
        cat(paste0(
            "\r",
            paste(treatment, annotation_type, context, sep = " - "), ":\t",
            round((n.map / length(map_ids)) * 100, 0), "% "
        ))
    }
    cat("\n")
}
...
cntx_loop <- c("all", "CG", "CHG", "CHH")
ann_loop <- c("Genes", "Promoters")

for (trnt in trnt_loop) {
    for (ann in ann_loop) {
        for (cntx in cntx_loop) {
            ec_squere_plots(treatment = trnt, annotation_type = ann, context = cntx)
        }
    }
}
