...
output_path <- "C:/Users/yonatany/Migal/Rachel Amir Team - General/yonatan/methionine/NGS_merged_results/TEGs_volcano_plots/RNAseq/"
...
RNAseq_volcano <- function(genotype) {
...
RNA_file <- read.csv(paste0("C:/Users/yonatany/Migal/Rachel Amir Team - General/yonatan/methionine/rnaseq_23/met23/",genotype,"/all_genes_results_",genotype,".csv")) %>%
...
vol_plot <- ggplot(mydf, aes_string(x = "log2FoldChange", y = "-log10(padj)", color = "Transposon_Super_Family")) +
...
svg(paste0(output_path, genotype, "_retro-TEGs_volcano.svg"),
...
for (genotype_loop in c("mto1_vs_wt", "mto3_vs_wt", "dCGS_vs_EV", "SSE_high_vs_EV", "SSE_low_vs_EV", "SSE_high_vs_SSE_low")) {
        try(RNAseq_volcano(genotype_loop))
