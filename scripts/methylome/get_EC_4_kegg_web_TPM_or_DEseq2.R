...
for (treatment in c("mto1", "mto3", "dCGS", "SSE_high", "SSE_low", "high_vs_low")) {
  for (by_folder in c("by_DEseq2")) { # by_TPM
    for (n.map in map_ids) {
      for (is.EC in c(TRUE)) { # FALSE
        ...
        treatment_name <- "SSE_high_vs_SSE_low"
        ...
        # RNA results
        # if (by_folder == "by_TPM") {
        #  RNA_file <- read.csv(paste0("C:/Users/yonatany/Migal/Rachel Amir Team - General/yonatan/methionine/rnaseq_23/raw_TPM_ttest/", treatment, "_TPM_ttest.csv"))[, c("locus_tag", "log2FC", "pValue")]
        #  names(RNA_file)[1] <- "gene_id"
        # } else
        if (by_folder == "by_DEseq2") {
          RNA_file <- read.csv(paste0("C:/Users/yonatany/Migal/Rachel Amir Team - General/yonatan/methionine/rnaseq_23/met23/", treatment_name, "/all_genes_results_", treatment_name, ".csv"))[, c("gene_id", "Symbol", "log2FoldChange", "pValue")]
          names(RNA_file)[3] <- "log2FC"
        }
        ...
        # name_if_ec <- ifelse(is.EC, "with_EC_", "")
        ...
        if (is.EC) {
          ...
          merged_tair <- merge.data.frame(ec_map, RNA_file, by = "gene_id") %>%
            dplyr::relocate(Symbol, .after = gene_id) %>%
            arrange(pValue) # %>% na.omit()
        } else {
          merged_tair <- merge.data.frame(tairs, RNA_file, by = "gene_id") %>%
            arrange(pValue) # %>% na.omit()
          # as.list(org.At.tairENZYME2TAIR)
        }

        # create directories
        path_name <- paste0("C:/Users/yonatany/Migal/Rachel Amir Team - General/yonatan/methionine/rnaseq_23/EC_kegg_maps/")
        dir.create(paste0(path_name, by_folder), showWarnings = F)
        dir.create(paste0(path_name, by_folder, "/", dir_if_ec), showWarnings = F)
        dir.create(paste0(path_name, by_folder, "/", dir_if_ec, treatment), showWarnings = F)
        dir.create(paste0(path_name, by_folder, "/", dir_if_ec, treatment, "/pathview"), showWarnings = F)

        # save RNA seq results for KEGG map
        write.csv(merged_tair, paste0(path_name, by_folder, "/", dir_if_ec, treatment, "/", treatment, "_values_", pathway_name, "_", name_if_ec, by_folder, ".csv"), row.names = F)
        ...
        setwd(paste0(path_name, by_folder, "/", dir_if_ec, treatment, "/pathview"))
        pathview(
          ...
          gene.idtype = "kegg",
          species = "ath",
          out.suffix = paste0(treatment, "_", pathway_name, "_", name_if_ec, by_folder),
          keys.align = "y",
          kegg.native = T,
          key.pos = "topright",
          sign.pos = "bottomleft",
        ...
        if (is.EC) {
          rmv_pos <- grep("gene_id|pValue|Symbol", names(merged_tair))
        } else {
          rmv_pos <- grep("pValue|Symbol", names(merged_tair))
        }
        ...
        merged_sig <- filter(merged_tair, pValue < 0.05) %>% .[,-rmv_pos] %>% arrange(desc(log2FC))
        merged_non_sig <- filter(merged_tair, pValue > 0.05) %>% .[,-rmv_pos]
        ...
        write.table(df_2_save, paste0(path_name, by_folder, "/", dir_if_ec, treatment, "/", treatment, "_", pathway_name, "_", name_if_ec, by_folder, ".txt"), row.names = F, col.names = F, sep = " ", quote = F)
      }
    }
  }
}
