...
genePlot_fun <- function(tair_id, var1_pool_f, var2_pool_f, methylome_at_annotations, methylome_at_results, output_path) {
  ...
  if (!is.null(methylome_at_results)) {
    ...
    chg <- rbind(
      read.csv(paste0(methylome_at_results, "/genome_annotation/CHG/Genes_CHG_genom_annotations.csv")),
      read.csv(paste0(methylome_at_results, "/genome_annotation/CHG/Promoters_CHG_genom_annotations.csv"))
    ) %>%
      select(all_of(columnNames)) %>%
      makeGRangesFromDataFrame(., keep.extra.columns = TRUE)

    chh <- rbind(
      read.csv(paste0(methylome_at_results, "/genome_annotation/CHH/Genes_CHH_genom_annotations.csv")),
      read.csv(paste0(methylome_at_results, "/genome_annotation/CHH/Promoters_CHH_genom_annotations.csv"))
    ) %>%
      select(all_of(columnNames)) %>%
      makeGRangesFromDataFrame(., keep.extra.columns = TRUE)
    # ) %>%
...
plotLocalMethylationProfile_yo <- function(
    methylationData1, methylationData2, region, DMRs = NULL,
  ...
  minPos <- start(region)
  maxPos <- end(region)
  methylationData <- .joinMethylationData(
    methylationData1,
    methylationData2
  )
  ...
  if (plotMeanLines) {
    ...
    maxColor <- max(c(
      movingAverageTotalReads1[!is.na(movingAverageTotalReads1)],
