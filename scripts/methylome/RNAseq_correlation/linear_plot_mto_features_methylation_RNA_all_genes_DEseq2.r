...
### filtered by all genes (no filters)
all_genes_list <- list(
  gene_list_name = "all_genes",
  gene_list = data.frame(gene_id = NULL)
)

### filtered by genes overlap with DMRs
DMRs_genes_list <- list(
  gene_list_name = "filtered_by_DMRs",
  gene_list = rbind(
    read.csv(paste0(DMRs_results_directory, "CG/Promoters_CG_genom_annotations.csv")),
    read.csv(paste0(DMRs_results_directory, "CG/Genes_CG_genom_annotations.csv")),
    read.csv(paste0(DMRs_results_directory, "CG/TEG_CG_genom_annotations.csv")),
    read.csv(paste0(DMRs_results_directory, "CHG/Promoters_CHG_genom_annotations.csv")),
    read.csv(paste0(DMRs_results_directory, "CHG/Genes_CHG_genom_annotations.csv")),
    read.csv(paste0(DMRs_results_directory, "CHG/TEG_CHG_genom_annotations.csv")),
    read.csv(paste0(DMRs_results_directory, "CHH/Promoters_CHH_genom_annotations.csv")),
    read.csv(paste0(DMRs_results_directory, "CHH/Genes_CHH_genom_annotations.csv")),
    read.csv(paste0(DMRs_results_directory, "CHH/TEG_CHH_genom_annotations.csv"))
  ) %>%
  distinct(gene_id)
)
...
gene_list_loop = list(all_genes_list, DMRs_genes_list, DEGs_genes_list)
...
i_gene_list = 1
while(i_gene_list <= length(gene_list_loop)) {

  gene_list = gene_list_loop[[i_gene_list]]$gene_list
  gene_list_name = gene_list_loop[[i_gene_list]]$gene_list_name
      ...
      tryCatch(
        {
          ...
          #  Meth     = seq(min(plot_df$Meth), max(plot_df$Meth), length.out = 100),
          #  genotype = unique(plot_df$genotype)
          # )
          #
          ## Get predictions on the response scale (the mean counts) plus standard errors
          # pred <- predict(lm_model_0, newdata = new_data, type = "response", se.fit = TRUE)
          ...
          # theta_df[theta_df$annotation == edit_ann_names(annotation_type), ]$theta_value = theta_value
          # theta_df[theta_df$annotation == edit_ann_names(annotation_type), ]$theta_se = theta_se

          #####################################################

          #### y-axis

          #y_max = max(plot_df$normCounts)
          #y_min = min(plot_df$normCounts)
          #y_mid = (y_max + y_min) / 2
          #y_mid_max = (y_mid + y_max) / 2
          #y_mid_min = (y_mid + y_min) / 2

          #### y-axis normalization
          # preds <- predict(lm_model_0, type = "response", se.fit = TRUE)
          # plot_df$fit = preds$fit
          # plot_df$se_fit = preds$se.fit
          # plot_df$lower = plot_df$fit - 1.96 * plot_df$se_fit
          # plot_df$upper = plot_df$fit + 1.96 * plot_df$se_fit

          ## Custom smoothing function
          # negbin_smooth <- function(formula, data, ...) {
          ...
          # geom_line(
          #  data = plot_df,
          #  aes(x = Meth, y = fit, color = genotype),
          #  size = 1
          # ) +
          ## Confidence ribbons around each fitted line
          # geom_ribbon(
          #  data = plot_df,
          #  aes(x = Meth, ymin = lower, ymax = upper, color = genotype),
          #  alpha = 0.2,
          #  color = NA
          # ) +
          # annotate("text", x = Inf, y = Inf, label = "M:G", hjust = 4.5, vjust = 6, size = 3.5) +
          # pValue text
          # annotate("text", x = Inf, y = Inf, label = paste0("'", MnG_pval, "'"), hjust = 1.15, vjust = 6, size = 3.5) +
          ...
          # write(lm_model, paste0(path_2_save.3, edit_ann_names(annotation_type), ".lm.stats.", context, ".", treatment, ".txt"))
          # }
        ...
        error = function(e) {
          message(paste0("\ncant use 'glm.nb' and create plot to - ", annotation_type, " annotations in ", context, " context\n"))
