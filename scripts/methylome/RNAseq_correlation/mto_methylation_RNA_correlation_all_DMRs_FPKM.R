...
for (treatment in c("mto1","mto3")) {
  for (context in c("CG","CHG","CHH")) {
    for (annotation_type in c("genes","promoters")) {
      ...
      path_2_save.0 = "P:/yonatan/methionine/NGS_merged_results/corr_with_methylations/by_FPKM/"
      ...
      fin_RNA = RNA_new %>%
        pivot_longer(!c(locus_tag,transcript_id), names_to = "Sample", values_to = "FPKM")
      ...
      if (
        ...
        fin_meth_RNA = cbind(fin_meth, fin_RNA[,c("transcript_id","FPKM")])
        ...
        cor_df = fin_meth_RNA %>%
          ...
          summarise(cor = stats::cor.test(Meth, FPKM, method="spearman", exact=F)$estimate,
                    pval = stats::cor.test(Meth, FPKM, method="spearman", exact=F)$p.value) %>%
          ungroup() %>% na.omit()
