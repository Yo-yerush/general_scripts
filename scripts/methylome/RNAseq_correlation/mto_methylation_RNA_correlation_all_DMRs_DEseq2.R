...
for (treatment in c("mto1","mto3")) {
  for (context in c("CG","CHG","CHH")) {
    for (annotation_type in c("genes","promoters")) {
      ...
      path_2_save.0 = "C:/Users/yonye/Migal/Rachel Amir Team - General/yonatan/methionine/NGS_merged_results/corr_with_methylations/by_DEseq2/"
      ...
      meth_matrix = read.csv(paste0("C:/Users/yonye/Migal/Rachel Amir Team - General/yonatan/methionine/methylome_23/BSseq_results_191223/average.meth.genes.levels/",treatment,"_vs_wt/",context,"/meth.",annotation_type,".",context,".",treatment,"_vs_wt.csv"))
      
      #Load the RNAseq '*.gene.results' files (output from RSEM pipeline)
      RNA.0 = read.csv(paste0("C:/Users/yonye/Migal/Rachel Amir Team - General/yonatan/methionine/rnaseq_23/met23/",treatment,"_vs_wt/norm.",treatment,"_vs_wt.DE.csv"))
      
      if (is.transcript) {
        names(RNA.0)[1] = "transcript_id"
        refseq_2_tair = read.csv("C:/Users/yonye/Migal/Rachel Amir Team - General/TAIR10.1/TAIR_2_refseq_transcript.csv")[,1:2]
        RNA = merge.data.frame(refseq_2_tair, RNA.0, by = "transcript_id")
      } else {
        names(RNA.0)[1] = "locus_tag"
        RNA = RNA.0
      }
      ...
      #  dplyr::select(locus_tag, transcript_id, everything())
      
      meth_new = meth_matrix_RNA[,c(grep("locus_tag",names(meth_matrix_RNA)), grep("meth",names(meth_matrix_RNA)))]
      RNA_new = meth_matrix_RNA[,c(grep("locus_tag",names(meth_matrix_RNA)), grep("RNA",names(meth_matrix_RNA)))]
      ...
      fin_meth = meth_new %>%
        pivot_longer(!c(locus_tag), names_to = "Sample", values_to = "Meth")
      
      fin_RNA = RNA_new %>%
        pivot_longer(!c(locus_tag), names_to = "Sample", values_to = "TPM")
      ...
      if (
        ...
        #  dplyr::select(locus_tag, everything())
        ...
        cor_df = fin_meth_RNA %>%
          group_by(locus_tag) %>% # check which column to group: 'locus_tag' or 'transcript_id'
