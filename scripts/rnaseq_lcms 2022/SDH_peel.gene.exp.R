...
plot.fun = data %>%
  ...
  labs(title = title_main, x = title_x, y = title_y, colour = col.data$exp) +
  
  # geom_jitter(color="steelblue4", size=1, alpha=0.9, width = 0.18) +   
  annotate("text", x = 5.5, y = max((y.labels.order)*(y.axis.hight)*0.962), label= p_value_lable, size = 7) + 
...
print(paste0(title_main," [",p_value_lable,"]"))

logo_file = "P:/yonatan/RNAseq_yonatan_2021/puni_legend.png"
ggdraw() + draw_plot(plot.fun) + 
  draw_image(logo_file, scale = 0.185,
             x = 1, y = 1.33, hjust = 0.64, vjust = 1)
#ggsave(paste("P:/yonatan/RNAseq_yonatan_2021/SDH_variants_RNAseq_yonatan/", title_main, " [",p_value_lable,"]", ".png", sep = ""), plot = plot.fun_2
