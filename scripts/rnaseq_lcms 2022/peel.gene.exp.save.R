...
if (p_value_lable == "ns") {
  p_ns = annotate("text", x = 5.5, y = max((y.labels.order)*(y.axis.hight)*0.95), label= p_value_lable, size = 5.5, fontface = 2)
} else {p_ns = annotate("text", x = 5.5, y = max((y.labels.order)*(y.axis.hight)*0.95), label= p_value_lable, size = 7)}
...
#plot(title_main)
dir.create(paste("P:/yonatan/saved.pics.R/peel_genes_", Sys.Date(), sep = ""))
ggsave(paste("P:/yonatan/saved.pics.R/peel_genes_", Sys.Date(),"/", transcript.id, title_main, ".png", sep = ""), 
       plot = plot.fun, width = 3.04, height = 3.85)
plot.fun
}
