...
# if there is a pthway name in the file from lc_pipline
  ...
  table = read.csv(file_for_metabo)[,c(1,4:11,15:17)]
names(table) = table[1,]
names(table)[1] = "X"
...
X = names(data)[1]
Y = names(data[2])
level.order = unique(data$X)[c(2,1,3,4)]

###############
t.test_p.value = t.test(data[1:5,2],data[6:11,2], var.equal = T)$p.value
...
if (p_value_lable == "ns") {
  p_ns = annotate("text", x = 2.5, y = max(data[,2])*1.05, label= p_value_lable, size = 5.5, fontface = 2)
} else {p_ns = annotate("text", x = 2.5, y = max(data[,2])*1.02, label= p_value_lable, size = 8.5)}
...
dir.create(paste(path_for_save_file,"/lcms_clone_", Sys.Date(), sep = ""))
  ...
  ggsave(paste(path_for_save_file,"/lcms_clone_", Sys.Date(),"/",pathway.name,"/", comp.name, ".png", sep = ""),
         plot = comp.plot, width = 2.5, height = 2.9)
