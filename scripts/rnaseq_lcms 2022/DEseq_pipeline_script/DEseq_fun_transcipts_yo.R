deseq_fc =  function(A.B_VS_c, # DE design. <"." for "&" (and)> <"_" for " " (space-bar)>
  ...
  if (PCA.only == F) {
    ...
    names(samples.deseq) = c("transcript_id", "log2FoldChange", "padj", "pValue")
    ...
    #  df_merge.pre = df_merge.pre[,c(1,4,2,3,5:8)]
    #} else {
      ...
      #}
      df_merge.pre = merge.data.frame(samples.deseq, transcript_id.and.description, by = "transcript_id", all.x = T)
      #df_merge.pre = df_merge.pre[,c(1,4,2,3)]
      df_merge.pre = df_merge.pre[order(df_merge.pre$padj, decreasing = F),]
      
    
    df_merge.pre_3 = df_merge.pre[df_merge.pre$padj <= 0.05,] ## only p<0.05
  ...
  if (PCA.only == F) {
    if (heatmap.plots ==T) {
      # heatmap
      file1 = data.frame(df_merge.pre$transcript_id)
