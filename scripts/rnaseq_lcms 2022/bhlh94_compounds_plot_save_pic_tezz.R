bhlh_compounds_plot_save = function(comp.name,
                               file_for_metabo = "P:/yonatan/RNAseq_yonatan_2021/lcms/lcms_results_060722/transgenic/transgenic_files/edited files for metabo/table_for_metaboanalyst_all.csv",
...
# if there is a pthway name in the file from lc_pipline
  if (file.exists("P:/yonatan/RNAseq_yonatan_2021/lcms/lcms_results_060722/transgenic/transgenic_files/files_for_metaboanalyst_18072022/comp.and.pathway.table.18072022.csv") == T) {
    comp.and.pathway.file = read.csv("P:/yonatan/RNAseq_yonatan_2021/lcms/lcms_results_060722/transgenic/transgenic_files/files_for_metaboanalyst_18072022/comp.and.pathway.table.18072022.csv")
...
table = read.csv(file_for_metabo)[c(1:4,6:11)]
names(table) = table[1,]
names(table)[1] = "X"
...
###############
t.test_p.value = t.test(data[1:5,2],data[6:9,2], var.equal = T)$p.value
...
if (nchar(comp.name) >= 40) {
  comp.name = paste(substring(comp.name, 1,30),"....", substring(comp.name, nchar(comp.name)-4,nchar(comp.name)), sep = "")
...
#########################
if (p_value_lable == "ns") {
 anno_ggplot = annotate("text", x = 1.5, y = max(data[,2])*1, label= p_value_lable, size = 5.5)
} else {
  anno_ggplot = annotate("text", x = 1.5, y = max(data[,2])*0.980, label= p_value_lable, size = 7)
}
########################
comp.plot = data %>%
  ggplot(aes(x=factor(x = !!sym(X), level=level.order), y = !!sym(Y))) +
  geom_boxplot(fill=c("#4688f2","grey70"), ) + 
...
dir.create(paste(path_for_save_file,"/lcms_bhlh94_", Sys.Date(), sep = ""))

  dir.create(paste(path_for_save_file,"/lcms_bhlh94_", Sys.Date(),"/",pathway.name, sep = ""))
  ggsave(paste(path_for_save_file,"/lcms_bhlh94_", Sys.Date(),"/",pathway.name,"/", comp.name, ".png", sep = ""),
         plot = comp.plot, width = 2.3, height = 2.45)
