clone.gene.exp = function(transcript.id,
                          path.to.clone_all.file = "P:/yonatan/RNAseq_yonatan_2021/DESeq2/clone/statistics/all_high_VS_low_no.C1.C5.C16/",
                          jitters = F)
{
library(dplyr)
library(ggplot2)
...
path = setwd(path.to.clone_all.file)

table = read.csv("norm.all_high_VS_low_no.C1.C5.C16.DE.csv")
table_padj = read.csv("all.transcripts.all_high_VS_low_no.C1.C5.C16.DE.csv")[,c(1,4)]
col.data = read.table("../../coldata.clone.txt", header = T)[-c(1,5,16),]
...
title_main = gsub("/","_",title_main)
####### for SDH
#SDH_n = readxl::read_excel("P:/yonatan/RNAseq_yonatan_2021/SDH_variants_RNAseq_yonatan/SDH_variants_RNAseq_yonatan.xlsx")[,c(1,6)]
#names(SDH_n)[2] = "sdh"
#sdh_name = SDH_n[SDH_n$transcript_id == gene.name,2]
#if (is.na(sdh_name) == T) {
#  sdh_name = " (SDH)"
#}
#title_main = paste(gene.name, sdh_name, sep = " ")
#######
...
level.order = c("Ban.G","101.2","RBP","116.17","Dan.A","WP")
###############
...
if (p_value_lable == "ns") {
  p_ns = annotate("text", x = 3.5, y = max((y.labels.order)*(y.axis.hight)*0.975), label= p_value_lable, size = 5.5, fontface = 2)
} else {p_ns = annotate("text", x = 3.5, y = max((y.labels.order)*(y.axis.hight)*0.95), label= p_value_lable, size = 7)}
