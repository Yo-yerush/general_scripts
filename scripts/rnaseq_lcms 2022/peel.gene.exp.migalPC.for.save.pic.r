peel.gene.exp.save = function(transcript.id, path.to.all_high_VS_low.folder = "P:/yonatan/RNAseq_yonatan_2021/DESeq2/peel/statistics/all_high_VS_low")
{
library(dplyr)
library(ggplot2)
library(ggsignif)
...
if (isTRUE(table[table$X == transcript.id,1] == transcript.id) == T) {
###
###
...
#############################
file.name = gsub(" ","_",unlist(table.names[table.names$transcript_id == gene.name,][,c(1,3)], use.names = F)[2])
file.name = gsub("-",".",file.name)


#if (file.exists(paste("P:/yonatan/saved.pics.R/", Sys.Date(), sep = "")) == F) {
  dir.create(paste("P:/yonatan/saved.pics.R/", Sys.Date(), sep = ""))
#} else { dir.create(paste("P:/yonatan/saved.pics.R/", Sys.Date(),"_2", sep = ""))}
ggsave(paste("P:/yonatan/saved.pics.R/", Sys.Date(),"/", transcript.id, file.name, ".png", sep = ""), plot = plot.fun)
plot.fun
} else {print(paste(transcript.id, "transcript is not exist in peel: high_VS_low files"))}
}