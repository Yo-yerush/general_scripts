clone.gene.exp = function(transcript.id,
                          res.directory = "all_high_VS_low_no.C1.C5.C16",
                          jitters = T)
{
library(dplyr)
library(ggplot2)
...
path = setwd(paste0("C:/Users/yonatany/Migal/Rachel Amir Team - General/yonatan/RNAseq_yonatan_2021/DESeq2/clone/statistics/", res.directory))

table = read.csv(paste0("norm.", res.directory, ".csv"))
...
plot.fun = data %>%
  ...
  geom_bar(stat = "summary",color="black" , fun.y = "mean", fill=c(rep("grey70",2),rep("#562842",2)), width=bar.width) +
