...
path_for_GO_analysis_results_folder = "P:/yonatan/RNAseq_yonatan_2021/GO_analysis/",
n.nodes = 5,
n.rows.results = 100,
save.files = T) {
...
allResBP <- GenTable(myGOdataBP, weightFisher= resultFisherBP, 
                     orderBy= "weightFisher", ranksOf= "weightFisher", topNodes=n.rows.results)
...
if (save.files == T) {
  ...
  write.csv(allResBP, paste(path_for_GO_analysis_results_folder,experiment,"_", treatment,
...
if (save.files == T) {
  ...
  write.csv(allResCC, paste(path_for_GO_analysis_results_folder,experiment,"_", treatment,
                            "/",experiment,"_", treatment,  ".",
                            GO_Ontology_type ,"." ,up_down,".topGO.csv", sep = ""),
            quote = F, row.names = F)
}
...
allResMF <- GenTable(myGOdataMF, weightFisher= resultFisherMF, orderBy= "weightFisher",
                     ranksOf= "weightFisher", topNodes=n.rows.results)
...
if (save.files == T) {
  ...
  write.csv(allResMF, paste(path_for_GO_analysis_results_folder,experiment,"_", treatment,
                            "/",experiment,"_", treatment,  ".",
                            GO_Ontology_type ,"." ,up_down,".topGO.csv", sep = ""),
            quote = F, row.names = F)
}
