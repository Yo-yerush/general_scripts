...
title_main = paste(unlist(table.names[table.names$protein_id == gene.name,][,c(1,3)], use.names = F)[2], " (",
                   unlist(table.names[table.names$protein_id == gene.name,][,c(1,3)], use.names = F)[1], ")",sep = "")
...
##
data$X = gsub("bHLH94","OE-bHLH94",data$X)
##
...
t.test_p.value = table_padj[table_padj$protein_id == gene.name,2]
...
plot.fun = data %>%
  ...
  geom_bar(stat = "summary",color="black" , fun.y = "mean", fill=c("#5c3e27", "grey70"), width=bar.width) +
...
dir.create(paste("P:/yonatan/saved.pics.R/bHLH94_genes_", Sys.Date(), sep = ""))
ggsave(paste("P:/yonatan/saved.pics.R/bHLH94_genes_", Sys.Date(),"/", transcript.id, title_main, ".png", sep = ""), plot = plot.fun,
       width = 3, height = 4.05)
plot.fun
}
