top.GO.fun = function(treatment = "high_VS_low",
  ...
  if (GO_Ontology_type == "BP") {
    ...
    if (save.files == T) {
      if (dir.exists(paste(path_for_GO_analysis_results_folder,experiment,"_", treatment,
                           sep = "")) == F) {
        dir.create(paste(path_for_GO_analysis_results_folder,experiment,"_", treatment, sep = ""))
      }
      write.csv(allResBP_view, paste(path_for_GO_analysis_results_folder,experiment,"_", treatment,
    ...
    setwd(paste(path_for_GO_analysis_results_folder,experiment,"_", treatment, sep = ""))
    printGraph(myGOdataBP, resultFisherBP, 
               firstSigNodes = n.nodes, fn.prefix= paste0(experiment,"_",treatment,"_BP_",up_down), useInfo="all", pdfSW= T)
    #printGraph(myGOdataBP, resultFisherBP, 
    ...
    if (save.files == T) {
      ...
      write.csv(allResCC_view, paste(path_for_GO_analysis_results_folder,experiment,"_", treatment,
                                     "/",experiment,"_", treatment,  ".",
                                     GO_Ontology_type ,"." ,up_down,".topGO.csv", sep = ""),
                quote = F, row.names = F)
    }
    ...
    setwd(paste(path_for_GO_analysis_results_folder,experiment,"_", treatment, sep = ""))
    printGraph(myGOdataCC, resultFisherCC, firstSigNodes = n.nodes, fn.prefix= paste0(experiment,"_",treatment,"_CC_",up_down), 
               useInfo="all", pdfSW= T)
    ...
    if (save.files == T) {
      ...
      write.csv(allResMF_view, paste(path_for_GO_analysis_results_folder,experiment,"_", treatment,
                                     "/",experiment,"_", treatment,  ".",
                                     GO_Ontology_type ,"." ,up_down,".topGO.csv", sep = ""),
                quote = F, row.names = F)
    }
    ...
    setwd(paste(path_for_GO_analysis_results_folder,experiment,"_", treatment, sep = ""))
    printGraph(myGOdataMF, resultFisherMF, firstSigNodes = n.nodes, fn.prefix= paste0(experiment,"_",treatment,"_MF_",up_down),
               useInfo="all", pdfSW= T)
  ...
  if (dotPlot) {
    ...
    bubble = allResMF_view %>% 
      ...
      labs(title = paste0("Molecular Function - corr (",positive_negative,") to ",treatment), x = "Annotated Genes", y = "") +
      theme(plot.title=element_text(hjust=0.5),
            legend.key.size = unit(0.45, 'cm'),
            legend.title = element_text(size=9.5)) + 
      geom_point()
    ggsave(paste0(path.to.save.files,"/",treatment,"_",GO_Ontology_type,"_dotPlot_",up_down,"_topGO.png"), 
           plot = bubble, width = 5.24, height = 4.86)
  }
