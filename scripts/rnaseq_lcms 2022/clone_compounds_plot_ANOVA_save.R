clone_compounds_plot_save = function(comp.name,
                               file_for_metabo = "P:/yonatan/RNAseq_yonatan_2021/lcms/lcms_results_060722/clone/clone_files/files_for_metaboanalyst_18072022/pathway.name.for.metabo.ANOVA.18072022.csv",
...
# if there is a pthway name in the file from lc_pipline
  if (file.exists("P:/yonatan/RNAseq_yonatan_2021/lcms/lcms_results_060722/clone/clone_files/files_for_metaboanalyst_18072022/comp.and.pathway.table.18072022.csv") == T) {
    comp.and.pathway.file = read.csv("P:/yonatan/RNAseq_yonatan_2021/lcms/lcms_results_060722/clone/clone_files/files_for_metaboanalyst_18072022/comp.and.pathway.table.18072022.csv")
...
X = names(data)[1]
Y = names(data[2])
level.order = unique(data$X)[c(3,2,1,4,6,5)]

###############
t.test_p.value = t.test(data[1:8,2],data[9:17,2], var.equal = T)$p.value
...
if (p_value_lable == "ns") {
  p_ns = annotate("text", x = 3.5, y = max(data[,2])*1.05, label= p_value_lable, size = 5.5, fontface = 2)
} else {p_ns = annotate("text", x = 3.5, y = max(data[,2])*1.02, label= p_value_lable, size = 8.5)}

#########################
comp.plot = data %>%
  ggplot(aes(x=factor(x = !!sym(X), level=level.order), y = !!sym(Y))) +
#  geom_boxplot(fill=c(rep("grey70",5),rep("#562842",5))) + 
  geom_bar(stat = "summary",color="black" , fun.y = "mean", fill=c(rep("grey70",3),rep("#562842",3)), width=bar.width) +
...
dir.create(paste(path_for_save_file,"/lcms_clone_", Sys.Date(), sep = ""))

  dir.create(paste(path_for_save_file,"/lcms_clone_", Sys.Date(),"/",pathway.name, sep = ""))
  ggsave(paste(path_for_save_file,"/lcms_clone_", Sys.Date(),"/",pathway.name,"/", comp.name, ".png", sep = ""),
         plot = comp.plot, width = 3.63, height = 4.21)
